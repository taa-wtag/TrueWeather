...
class GetForecastWeatherUseCase @Inject constructor(
    ...
    suspend fun getWeatherDay(city: String): Flow<List<DailyWeatherItem>> {
        ...
        current.firstOrNull()?.let { if(it.isNotEmpty() && it.size>=FORECAST_MAX_DAYS-1) return current }
        getWeatherFromRemote(city)
    ...
    private suspend fun getWeatherFromRemote(city: String) {
        ...
        weather?.currentWeatherData?.toHourlyWeatherItem()?.let {weatherRepository.addWeather(city,it)}
        weather?.forecastData?.dailyForecastDataList?.forEach { day ->
            weatherRepository.addWeather(city,day.toHourlyWeatherItem())
        }
        weather?.forecastData?.dailyForecastDataList?.map { it.toDailyWeatherItem() }.run { weatherRepository.addWeather(city,this) }
    }
}