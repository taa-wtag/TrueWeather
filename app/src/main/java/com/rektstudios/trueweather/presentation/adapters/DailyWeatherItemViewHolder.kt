...
class DailyWeatherItemViewHolder(private val binding: ItemWeatherDayBinding) :
    ...
    fun bind(dailyWeatherItem: DailyWeatherItem, glide: RequestManager) {
        if (dailyWeatherItem.isValid) {
            ...
            glide.load(dailyWeatherItem.imageUrl).into(binding.imageViewWeatherDayCondition)
        }
    }
}